    ...
    def __init__(self, dim, window_size=7, use_attention=True, use_pos_encoding=True):
        ...
        if use_attention:
            ...
            self.conv = nn.Sequential(
                nn.Conv2d(dim, dim, 3, padding=1, bias=False), nn.BatchNorm2d(dim), nn.ReLU(inplace=True)
            )
...
class CrossScaleAttention(nn.Module):
    def __init__(self, in_main, in_small, use_attention=True, use_pos_encoding=True):
        ...
        self.gate = nn.Sequential(
            nn.Conv2d(in_small * 2, in_small, 1, bias=False), nn.BatchNorm2d(in_small), nn.Sigmoid()
        )
...
class YOLOv11SmallObjectDetector(nn.Module):
    def __init__(
        ...
        # 构建backbone
        self.backbone = BackboneWithSmallBranch(use_small_branch=use_small_branch, use_ghost=use_ghost)

        # 构建融合模块
        if use_cross_scale_fusion and use_small_branch:
            self.fusion = CrossScaleAttention(
                in_main=128, in_small=128, use_attention=use_attention, use_pos_encoding=use_pos_encoding
            )
        else:
    ...
    def forward(self, x, teacher_feats=None, teacher_output=None):
        ...
        if self.use_teacher:
            ...
            if teacher_output is not None:
                ...
                t_logits = (
                    teacher_output.view(teacher_output.size(0), teacher_output.size(1), -1).permute(0, 2, 1).detach()
                )

                # 优化：使用更轻量的蒸馏损失计算
                # 只计算置信度和类别的蒸馏，不计算回归分支
                loss_output = self.kl(
                    F.log_softmax(s_logits[..., : self.nc], dim=-1), F.softmax(t_logits[..., : self.nc], dim=-1)
                )
